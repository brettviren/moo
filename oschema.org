...
In general, a *schema* describes one or more *types*.  There are many ways
to represent schema (JSON Schema, XML schema, Protobuf or Avro
schema).  In *moo* the /oschema/ representation is one that is supported.
The "o" stands for "object" as /oschema/ has an object representation.
Any data or programming language which provides support for the
concepts described here and which can provide compatible data
structures may be used by *moo*.

A schema expressed as /oschema/ defines a number of *types*.  Each type is
an instance of exactly one *schema class*.  There are a limited number
of schema classes defined:
...
Every /oschema/ *type* then provides a set of *attributes* as determined by
its schema class.  Some attributes are common across all schema
classes and are required or optional:
...
The types of some schema classes must *reference* one or more other
types.  These include instances of: ~sequence~, ~record~, ~namespace~, and
~anyOf~.  A *type reference* is represented as a /fully-qualified type name/
(FQTN) which is formed as the period ("~.~") delimited concatenation of
the elements of ~path~ (if any) and the ~name~.  A FQTN shall not begin
nor end with a period and the concept of a "relative" type reference
is not supported.
...
This section describes how to author a schema in either the *Jsonnet* or
*Python* language.  Too illustrate some patterns of large scale projects
the example will factor the schema into two parts
...
*moo* /oschema/ may be described easily in the [[https://jsonnet.org/][Jsonnet]] language as it
explicitly targets defining data structures.
...
This call of the ~schema()~ function of the ~moo.oschema~ object returns
another object held by ~sys~ which will provide sort of a "schema
factory" that operates "in" the path of ~sys~.  We see it in action:
...
This last line defines the data structure that is sort of the "return"
value of the ~sys.jsonnet~ file.  That is, this file "compiles" to an
...
We can see how the /sys/ schema expands or compiles to its JSON
...
object structure.  These details will need to be understood if any new
native language support is developed.
#+end_note
