...
In general, a *schema system* (aka meta schema) defines ways in which we
may define *schema*.  Each schema describes one or more *types*.  And each
type in turn defines the allowed structure and constraints of *models*
(aka "values").  *moo* supports a number of schema systems but the
dominant one is /oschema/.
...
The *moo* /oschema/ schema classes are:
...
Some *types* must *reference* one or more other types.  These include
instances of the schema classes: ~sequence~, ~record~, ~namespace~,
and ~anyOf~.  A *type reference* is represented as a /fully-qualified
type name/ (FQTN) which is formed as the dot-separated concatenation
of the elements of ~path~ (if any) and the ~name~.  A FQTN shall not
begin nor end with a period.  As the ~path~ is absolute there is no
...
*Jsonnet* or the *Python* programming language as a persistent
