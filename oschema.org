...
#+begin_src shell :exports both :results output code :wrap "example"
  moo -J examples/oschema -t moo.oschema.typify dump -f pretty app.jsonnet
#+end_src
...
[<Any "app.Affiliation">,
 ...
 <String "app.Email">,
 <Record "app.Person" fields:{email, counts, affil}>]
#+end_example

The built-in ~moo.oschema.typify()~ function illustrated here takes an
oschema (array of types) and constructs instances of Python versions
of the oschema schema classes which provide additional functionality.
The template may then rely on the resulting object-oriented structure.
...
Note the ~Person~ type comes after the types that it refers to in its fields.

This example also implies that a pipeline of transformations may
...
We want the development process to be a vehicle to achieve these goals:

- [ ] Provide an example "oschema" array of types as one Jsonnet file.
- [ ] Apply ~moo~ transforms to produce a dependency-ordered array of ~moo.schema~ instances.
- [ ] Define a model-template contract and develop both ~ostructs~ and ~onljs~ against it.
- [ ] Automate and ensconce any complex bits into ~moo~ Python, Jsonnet or Jinja support code.

Some features of the resulting generated code that are desired:

- [ ] A reference to a type should not include ~namespace~ qualification
  when that type is defined in the same ~namespace~ as the referees is
  used.  Ie, use only ~Person~ and not ~app::Person~ when referring to the
  type from inside ~namespace app~.  This is purely for code looks.
- [ ] A leading ~namespace~ must be specified and only types in that
  ~namespace~ defined.  
- [ ] Any type references outside that ~namespace~ must be satisfied at
  the C++ level with a suitable ~#include~.  The *template will assume* a
  convention to map ~namespace~ (aka oschema ~path~) to include files.
...
#+include: examples/oschema/omodel.jsonnet src jsonnet
...
We want to apply a transform to the ~types~ attribute and can test that with:
...
#+begin_src shell :exports both :results output code :wrap "src python"
  moo -g '/lang:ocpp.jsonnet' \
      -T examples/oschema -J examples/oschema \
      -A os=app.jsonnet -A path=app \
      -t '/types:moo.oschema.typify|moo.oschema.graph|moo.oschema.depsort' \
      dump -f pretty omodel.jsonnet
#+end_src
...
{'byscn': {'any': ['app.Affiliation'],
 ...
 'lang': {'dtypes': {'f4': 'float',
                     ...
                     'u4': 'uint32_t',
                     'u8': 'uint64_t'},
          'imports': {'any': ['nlohman/json.hpp'],
                      'anyof': ['variant'],
                      'sequence': ['vector'],
                      'string': ['string']},
          'types': {'any': 'nlohman::json',
                    'sequence': 'std::vector',
                    'string': 'std::string'}},
 'nspre': 'app.',
 'path': ['app'],
...
The [[file:examples/oschema/ostructs.hpp.j2][ostructs.hpp.j2]] template file gets applied to the ~omodel~ to
...
namespace app {
    ...
    using Affiliation = nlohman::json;
