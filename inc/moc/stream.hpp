...
namespace moc {
    ...
    // A moc data delivery starts at a byte stream level.
    using octet = char;

    // On top of a byte stream is a frame stream.
    using frame_t = std::vector<octet>;

    // A frame stream is provided by anything that looks like this
    // function.
    using stream_source_t =
        std::function<frame_t()>;

    /// moc provides a frame stream from "sized, hinted and framed
    /// message" (SHFM) byte stream.
    struct shfm_source {
        std::istream& stream;
    ...
    // moc provides a frame stream from a JSON stream.
    struct json_source {
        std::istream& stream;
        size_t count{0};
        ...
        frame_t operator()() {
            if (stream.eof()) {
                // fixme: mock: better to have a specific exception here.
                throw std::runtime_error("EOF in JSON stream");
            }
            ++count;
            if (count%2 == 1) {
            ...
            try {
                ...
                if (pe.id == 101) {
                    // convert to common exeption type.
                    // fixme: moc: best to have a specific exception here.
                    throw std::runtime_error("Parse error in JSON stream");
                }
                throw;          // dunno what it is
            }
    ...
    /// fixme: Provide a SHFM stream from  ZeroMQ messages
    // struct zeromq_source {};


    // A moc "type stream" consumes frames from a frame stream source
    // and returns typed objects.  This relies on the application also
    // including low-level, typed serialization codec code such as
    // generated by moc for nlohmann::json (and eventually avro)
    class type_stream {
    ...
    // Factory method to return a type stream given a particular type
    // of byte stream.
    inline
