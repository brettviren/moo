...
In the above we give ~load_types()~ a file system path to find the *moo*
test suite area relative to where we execute the example code (assumed
in top-directory of *moo* git repo).  *moo* will search this path when a
file is given as a relative path.  The current working directory ("~.~")
will also be checked and failing that, any directories listed in the
environment variable ~MOO_MODEL_PATH~ will be searched.
