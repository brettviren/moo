...
Okay, how to use it.  Let's start with a non-functional top-level code:

#+include: examples/tla/notla.jsonnet src jsonnet

This is somewhat contrived.  The ~var~ will be our variable of interest.
The ~result~ represents some intermediate structure construction,
possibly very complex in a real world use.  The last line provides the
value of the file as a whole.
...
To refactor we define a top-level ~function~ and move ~var~ into its
argument list and its body holds the intermediate structure
construction:

#+include: examples/tla/tla.jsonnet src jsonnet

There are other ways to shape the function body.  What is shown here
is the use of an "inside-out pattern".  The inner ~result~ value is used
to hold the final construction in the context of an object.  We then
set another value called ~return~ which is exposed as the value of the
~function~.  This is admittedly overkill for this simple example but
shows a common pattern used when the structure requires various
intermediates.
...
And, finally, we may see how to provide a variant value:
...
And one more example shows how we may reuse this same ~tla.jsonnet~ to
build some higher-level structure.
...
Some examples show the power.  First we may write some Jsonnet code
...
#+include: examples/tla/passthrough.jsonnet src xml
...
There's almost never a wrong place to structure Jsonnet as a top-level
function.  Even if one expects no use of TLAs the ~function()~ may have
an empty argument list.  The only "cost" to the developer is wrapping
what would otherwise be the return value in ~function()~, maybe making
use of the "inside-out pattern".

The real effort is in factoring the body so that any abstracted
parameters may be provided by TLAs.  This factoring may lead to more
Jsonnet files holding yet smaller functions.  Thus is the nature of
well factored, functional programming.

In applying TLA one should be sensitive to how the abstracted
information will ultimately be provided.  In the context of a [[file:buildsys.org][build
system]] it may go into, eg, CMake files and one should evaluate if
adding complexity there makes sense.  Or, one may provide one or more
layers of outer Jsonnet scope to "whittle down" the amount of
abstracted parameters by providing sane defaults.  An example from *moo*
is how the [[file:oschema][omodel]] is used to connect simple array of type schema to an
overall schema expected by some of the codegen templates.
